...
| **CABIO-test** | `/workspaces/qte77/CABIO-test` | Business pipeline orchestrator. BRD->PRD->FRP generation. Multi-project, multi-company dashboard and cost tracking. | cc-utils-plugin (all plugins including market-research) |
| **Ralph WT template** | `/workspaces/qte77/ralph-loop-cc-tdd-wt-vibe-kanban-template` | Coding engine. Takes PRD/FRP, runs TDD loops in git worktrees, produces working `src/` + `tests/`. | cc-utils-plugin (python-dev, ralph plugins) |
...
CABIO (business pipeline)
  Business Input → BRD → [gate] → PRD → [gate] → FRP fan-out
  │                                                    │
  │ each FRP becomes a PRD for Ralph                   │
  ▼                                                    ▼
Ralph WT (coding pipeline)                    Ralph WT (coding pipeline)
  FRP-as-PRD → prd.json → TDD loop             FRP-as-PRD → prd.json → TDD loop
...
| **Ralph WT** | Make business decisions. Executes whatever PRD/FRP it receives. |
...
CABIO's FRP output becomes Ralph's PRD input:
1. CABIO generates `context/FRPs/project_feature.md`
2. Cockpit copies/transforms FRP into Ralph's `docs/PRD.md` format
...
| FRP generation | `claude -p /project:generate-frp-from-prd` | CC bridge `total_cost_usd` |
